...
// ---------------------------------------------------------
// 1. IBM Exact Analytical Solution
// ---------------------------------------------------------
...
// ---------------------------------------------------------
// 2. Python Plotting Script Generator
// ---------------------------------------------------------
...
// ---------------------------------------------------------
int main(){
    std::cout << "Starting IBM Benchmark Solver...\n";
    ...
    for (int n_elem = 20; n_elem <= 100; n_elem += 80) { 
        ...
        std::vector<double> element_k(msh.n_elements, 0.0);
        double R = 0.5; 

        for (int i = 0; i < msh.n_elements; ++i) {
            auto conn = msh.Elements[i];
            double xc = (msh.Coords[conn[0]].first + msh.Coords[conn[1]].first + 
                         msh.Coords[conn[2]].first + msh.Coords[conn[3]].first) / 4.0;
            double yc = (msh.Coords[conn[0]].second + msh.Coords[conn[1]].second + 
                         msh.Coords[conn[2]].second + msh.Coords[conn[3]].second) / 4.0;
            double rc = std::sqrt(xc*xc + yc*yc);
            
            if (rc < R) element_k[i] = 1.0;  
            else        element_k[i] = 10.0; 
        }

        auto dynamic_source = [](double x, double y) { return 0.0; };

        flow.DirichletEdges.resize(2); 
        flow.DirichletEdges[0] = {0, 1, 2, 3}; // Select all 4 walls
        flow.DirichletEdges[1] = {};           // Empty 2nd DOF
        ...
        assemble_matrices_ibm(msh, flow, element_k, dynamic_source);
