// ========================================
// File: main.cpp 
// Immersed Boundary Method Benchmark
// ========================================
#ifndef M_PI
...
int main(){
    ...
    for (int n_elem = 20; n_elem <= 100; n_elem += 80) { 
        ...
        double h_mesh = 2.0 / n_elem;         // grid spacing (domain is [-1,1] => length 2)
        double eps    = 2.0 * h_mesh;         // interface half-width: spans ~2 elements

        for (int i = 0; i < msh.n_nodes; ++i) {
            double x = msh.Coords[i].first;
            double y = msh.Coords[i].second;
            double r = std::sqrt(x*x + y*y);
            double d = r - R;                  // signed distance: negative inside, positive outside

            if      (d < -eps) node_phi[i] = 1.0;
            else if (d >  eps) node_phi[i] = 0.0;
            else               node_phi[i] = 0.5 * (1.0 - d/eps - (1.0/M_PI) * std::sin(M_PI * d / eps));
        }
